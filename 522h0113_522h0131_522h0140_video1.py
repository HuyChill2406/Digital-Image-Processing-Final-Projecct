...
def process_frame(frame):
    ...
    detect_shapes(frame, masks['red'], masks['blue'], masks['white'],masks['blue2'])
    
    return frame

def detect_shapes(frame, red_mask, blue_mask,white_mask,blue2_mask):
    """
    Phát hiện hình dạng và phân loại các biển báo dựa trên các mặt nạ màu.
    """
    masks = [red_mask, blue_mask,white_mask,blue2_mask]
    ...
    for mask in masks:
        ...
        for cnt in contours:
            ...
            if area > 1000:  # Bỏ qua các đối tượng nhỏ
                x, y, w, h = cv2.boundingRect(cnt)
                circularity = (4 * np.pi * area) / (cv2.arcLength(cnt, True) ** 2)
                classify_sign(frame, x, y, w, h, cnt, red_mask, blue_mask,white_mask,blue2_mask,circularity,area)

def classify_sign(frame, x, y, w, h, cnt, red_mask, blue_mask,white_mask,blue2_mask,circularity,area):
    """
    ...
    total_pixels = w * h if w * h > 0 else 1
    
    red_ratio = red_pixels / total_pixels
    blue_ratio = blue_pixels / total_pixels
    white_ratio = white_pixels / total_pixels
    if area > 2000 and blue2_pixels > red_pixels and circularity > 0.5 and is_rectangle(cnt):                   
        label_object(frame, "BIEN BAO CHI DAN", x, y, x + w, y + h)
    elif circularity > 0.7:
        if red_ratio > 0.6:
            label_object(frame, "CAM DI NGUOC CHIEU", x, y, x + w, y + h)
        elif blue_ratio > 0.6:
            label_object(frame, "PHAI DI VONG SANG PHAI", x, y, x + w, y + h)
        elif white_ratio > 0.001:
            label_object(frame, "CAM RE TRAI", x, y, x + w, y + h)
        elif blue_ratio > 0.2:
            label_object(frame, "CAM DO XE", x, y, x + w, y + h)
        else:
            label_object(frame, "CAM DUNG VA DO XE", x, y, x + w, y + h)
...
def video1():
    ...
    output_video_path = '522h0113_522h0131_522h0140_video1.avi'
    ...
    student_ids = ["Student ID: 522h0131", "Student ID: 522h0140", "Student ID: 522h0113"]
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
            break
        ...
        cv2.imshow("Video 1 (PRESS 'q' TO QUIT)",processed_frame)
